...
!pdac------------
     ...
     &        2.16178D0*DEXP(-2.43787D0*tst)
        mmug(ik)=26.69D0*(gmw(ik)*tg)**0.5D0
     &          /(mmugs(ik)**2*om)
      END DO
!
...
!         
         ...
         tyx2=(ug(ijp)-ug(ij))*indzp*2.D0+
     &        (vg(ipj)-vg(ij))*indrp*2.D0
         tyx1=(ug(ij)-ug(ijm))*indzm*2.D0+
     &        (vg(ipjm)-vg(ijm))*indrp*2.D0
         tyy2=2.D0*mugt(ijt)*(vg(ijp)-vg(ij))*indz(j+1)
         tyy1=2.D0*mugt(ij)*(vg(ij)-vg(ijm))*indz(j)
         txy2=tyx2
         txy1=(ug(imjp)-ug(imj))*indzp*2.D0+
     &        (vg(ij)-vg(imj))*indrm*2.D0
...
! 
         IF(itc.EQ.1) THEN
           ...
           t0=t0-2.D0/3.D0*gmum*((ugm2-ugm1)*indrp*2.D0*inrb(i)+
     &        (vgm2-vgm1)*indz(j))
         ENDIF
!
     ...
     &      +(vg(ijp)-vg(ij))*indz(j+1))
         tyy1=tyy1-d1c
         vmu21=(dz(j)*ep(ijt)*mugt(ijt)+
     &        dz(j+1)*ep(ij)*mugt(ij))*indzp
         vmu22=(dz(j)*ep(ijtr)*mugt(ijtr)+
     &        dz(j+1)*ep(ijr)*mugt(ijr))*indzp
         vmu2=(dr(i+1)*vmu21+dr(i)*vmu22)*indrp
         vmu11=(dz(j-1)*ep(ij)*mugt(ij)+
     &        dz(j)*ep(ijb)*mugt(ijb))*indzm
         vmu12=(dz(j-1)*ep(ijr)*mugt(ijr)+
     &        dz(j)*ep(ijbr)*mugt(ijbr))*indzm
...
!
     ...
     &             *indrp*2.D0*inrb(i) 
     &            +(vmu2*tyx2-vmu1*tyx1)*indz(j)-t0*vdu*inrb(i)
         vmu11=(dz(j)*ep(ijtl)*mugt(ijtl)+
     &        dz(j+1)*ep(ijl)*mugt(ijl))*indzp
         vmu1=(dr(i)*vmu11+dr(i-1)*vmu21)*indrm
         gvisz(ij)=(rb(i)*vmu2*txy2-rb(i-1)*vmu1*txy1)
     &           *inr(i)*indr(i)
...
!         
         ...
         tyx2=(uk(k,ijp)-uk(k,ij))*indzp*2.D0+
     &        (vk(k,ipj)-vk(k,ij))*indrp*2.D0 
         tyx1=(uk(k,ij)-uk(k,ijm))*indzm*2.D0+
     &        (vk(k,ipjm)-vk(k,ijm))*indrp*2.D0
         tyy2=2.D0*mus(k,ijt)*(vk(k,ijp)-vk(k,ij))*indz(j+1)
         tyy1=2.D0*mus(k,ij)*(vk(k,ij)-vk(k,ijm))*indz(j)
         txy2=tyx2
         txy1=(uk(k,imjp)-uk(k,imj))*indzp*2.D0
     &       +(vk(k,ij)-vk(k,imj))*indrm*2.D0
         txx2=txx2-2.D0/3.D0*mus(k,ijr)*
     $       ((rb(i+1)*uk(k,ipj)-rb(i)
     $       *uk(k,ij))*inr(i+1)*indr(i+1)
...
!
         IF(itc.EQ.1) THEN
           gmum=(dr(i)*mus(k,ijr)*rlk(k,ijr)
     $         +dr(i+1)*mus(k,ij)*rlk(k,ij))*indrp*inrl(k)
           ...
           t0=t0-2.D0/3.D0*gmum*((ukm2
     $       -ukm1)*2.D0*indrp*inrb(i)+(vkm2-vkm1)*indz(j))
         ENDIF
!
...
!
         ...
         ENDIF
!
         vmu21=(dz(j)*mus(k,ijt)*rlk(k,ijt)+
     $        dz(j+1)*mus(k,ij)*rlk(k,ij))*indzp
         vmu22=(dz(j)*mus(k,ijtr)*rlk(k,ijtr)+
     $        dz(j+1)*mus(k,ijr)*rlk(k,ijr))*indzp
         vmu2=(dr(i+1)*vmu21+dr(i)*vmu22)*indrp*inrl(k)
         vmu11=(dz(j-1)*mus(k,ij)*rlk(k,ij)+
     $        dz(j)*mus(k,ijb)*rlk(k,ijb))*indzm
         vmu12=(dz(j-1)*mus(k,ijr)*rlk(k,ijr)+
     $        dz(j)*mus(k,ijbr)*rlk(k,ijbr))*indzm
         vmu1=(dr(i+1)*vmu11+dr(i)*vmu12)*indrp*inrl(k)
!
         pvisx(k,ij) = (rlk(k,ijr)*txx2*r(i+1)
     $                     -rlk(k,ij)*txx1*r(i))*indrp*2.D0
     $                     *inrb(i)*inrl(k)+
...
!
         pvisz(k,ij) = (rb(i)*vmu2*txy2-rb(i-1)*vmu1*txy1)
     $                     *inr(i)*indr(i)
     $                     +(rlk(k,ijt)*tyy2-rlk(k,ij)*tyy1)
     &                     *indzp*2.D0*inrl(k)
        END IF
      END DO
!
