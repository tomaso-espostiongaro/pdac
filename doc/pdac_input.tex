...
\subsection{\OBJ{control} namelist}
...
\item
\PDACCONFWDEF{ift}{flag for time-dependent input}{0-5}{1}
{Function type used to change the mass flow rate at the inlet (if \OBJ{tau1} 
or \OBJ{tau2} $> 0.0$)}
\end{itemize}

\subsection{\OBJ{model} namelist}
...
\subsection{\OBJ{boundaries} namelist}
...
\subsection{\OBJ{topography} namelist}
...
{When \OBJ{itp} = 1, read the topography from a file.}
...
\item
\PDACCONFWDEF{min\_angle}{minimum angle for averaging}{0-180}{0}
{Minimum angle, in degrees, to start axisymmetric averaging (see \OBJ{iavv}
flag.}

\item
\PDACCONFWDEF{max\_angle}{maximum angle for averaging}{0-180}{0}
{Maximum angle, in degrees, to start axisymmetric averaging (see \OBJ{iavv}
flag.}
...
\item
\PDACCONFWDEF{seatable}{Flag for removing the bathimetry} {T/F}{T}
{When .TRUE., it sets to sea level (0) all negative elevations (e.g.,
reporting the bathimetry).}

\item
\PDACCONFWDEF{nocrater}{Flag for crater filling}{T/F}{F}
...
the \OBJ{nocrater} option is selected, the quota can be set by the user
...
{If \OBJ{nocrater} is .TRUE., the topography is flattened at this quota.}
...
{This value is usually set equal to the \OBJ{dxmin} or \OBJ{dymin} values}
...
\subsection{\OBJ{inlet} namelist}
The flow conditions at the volcanic vent can be specified
explicitly (i.e., cell-by-cell) in the \OBJ{'FIXED\_FLOWS'} 
card for both 2D and 3D simulations.
Alternatively, the \OBJ{inlet} namelist can be used to impose the vent
...
enclosed within the \OBJ{vent\_radius}. Nevertheless, some of the
...
controlled by the \OBJ{tau}, \OBJ{tau1}, \OBJ{tau2} parameters in the
\OBJ{control} namelist. 
...
if \OBJ{itp}=1. Since the topography may be irregular around the vent
center, it is flattened within a distance defined by the parameter
\OBJ{base\_radius} defined below.}
...
the antialias procedure (flat \OBJ{iali} above).}
...
\OBJ{base\_radius} must lay on a uniform mesh region.}
...
{If \OBJ{ipro} is set to 1, this value is overwritten. This value
is also modified by the flag \OBJ{wrat}.}
...
{If \OBJ{ipro} is set to 1, this value is overwritten. This value
is also modified by the flag \OBJ{wrat}. On the vent perimeter,
it is modified when \OBJ{iali}=3 or \OBJ{irand}=1 (see descriptions)
...
{If \OBJ{ipro} is set to 1, this value is overwritten. 
This value is modified on the vent perimeter when \OBJ{iali}=1,
to adjust the local density and mass flow rate.}

\item
\PDACCONFWDEF{t\_gas}{Temperature at the vent}{any real $>$ 0.0}{288.15}{If \OBJ{ipro} is set to 1, this value is overwritten. }
...
{The values for different particle classes must be separated by commas.
If \OBJ{ipro} is set to 1, this value is overwritten.
This value is modified on the vent perimeter when \OBJ{iali}=1/2,
to adjust the local density and mass flow rate.  }
...
\item
\PDACCONFWDEF{vent\_O2}{Averaged concentration of O2 at the vent}{any real [0.0:1.0]}{0.0}{If \OBJ{ipro} is set to 1, this value is overwritten.}

\item
\PDACCONFWDEF{vent\_N2}{Averaged concentration of N2 at the vent}{any real [0.0:1.0]}{0.0}{If \OBJ{ipro} is set to 1, this value is overwritten.}

\item
\PDACCONFWDEF{vent\_CO2}{Averaged concentration of CO2 at the vent}{any real [0.0:1.0]}{0.0}{If \OBJ{ipro} is set to 1, this value is overwritten.}

\item
\PDACCONFWDEF{vent\_H2}{Averaged concentration of H2 at the vent}{any real [0.0:1.0]}{0.0}{If \OBJ{ipro} is set to 1, this value is overwritten.}

\item
\PDACCONFWDEF{vent\_H2O}{Averaged concentration of H2O at the vent}{any real [0.0:1.0]}{1.0}{If \OBJ{ipro} is set to 1, this value is overwritten.}

\item
\PDACCONFWDEF{vent\_Air}{Averaged concentration of Air at the vent}{any real [0.0:1.0]}{0.0}{If \OBJ{ipro} is set to 1, this value is overwritten.}

\item
\PDACCONFWDEF{vent\_SO2}{Averaged concentration of SO2 at the vent}{any real [0.0:1.0]}{0.0}{If \OBJ{ipro} is set to 1, this value is overwritten.}
\end{itemize}

\subsection{\OBJ{dome} namelist}
...
Dome cells are automatically imposed over the topography if \OBJ{ibl=1}.
...
\item
\PDACCONFWDEF{zdome}{Elevation of the center of the dome}{any real}{0.0}
{If \OBJ{zdome} is lower than the actual topography elevation, the 
...
{The dome pressure is constant within this distance from the dome center.}

\item
\PDACCONFWDEF{overpressure}{Constant overpressure imposed within the dome}{any real $>$ 0.0}{100.D5}
{This value is used when \OBJ{idome} = 2}

\item
\PDACCONFWDEF{particle\_fraction}{particle volumetric fractions}{array of real [0.0:1.0]}{0.7}
{The size of the array equals {\tt nsolid}. 
The sum of the particle volumetric fractions must not exceed 1.0}

\item
\PDACCONFWDEF{gas\_flux}{Gas flux from the conduit}{any real $>$ 0.0}{400.0}
{This value is used to compute the porous dome pressure distribution when
\OBJ{idome} is set to 1.}

\item
\PDACCONFWDEF{temperature}{Dome temperature in Kelvin}{any real $>$ 0.0}{1100.D0}
{Dome temperature is uniform.}
...
\item
\PDACCONFWDEF{rocks\_volume}{Volume of the dome external shell}{any real $>$ 0.0}{0.0}{Volume of the shell of lithic material surrounding the dome core.}

\item
\PDACCONFWDEF{overpressure\_rocks}{Overpressure within the external shell}{any real $>$ 0.0}{0.0}{}

\item
\PDACCONFWDEF{temperature\_rocks}{Temperature within the external shell}{any real $>$ 0.0}{0.0}{}

\item
\PDACCONFWDEF{particle\_fraction\_rocks}{particle volumetric fractions in the external shell}{array of real [0.0:1.0]}{0.0}
...
\subsection{\OBJ{atmosphere} namelist}
...
\subsection{\OBJ{particles} namelist}
...
\item
\PDACCONFWDEF{twophase\_limit}{limit of matrix solution for particles}{real}{1.D-8}
{Depends on materials}

\end{itemize}

\subsection{\OBJ{numeric} namelist}
...
\subsection{\OBJ{'FIXED\_FLOWS'} card}
